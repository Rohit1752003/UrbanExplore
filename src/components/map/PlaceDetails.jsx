...
const PlaceDetails = ({ place }) => {
  ...
  return (
    <div>
      <div
        className="w-full rounded-sm bg-white shadow-md overflow-hidden mb-5 relative z-10 cursor-pointer"
      ...
      >
        ...
        <div className="absolute top-0">
          ...
          {!place?.isClose && place?.open_now_text && (
            // If Place isClose - a red button is displayed else a green button is displayed - this is possible with the difference in classes as seen below
            <button
              className={`${place?.open_now_text?.includes("Closed") ? "bg-red-400" : "bg-green-400"} rounded-full py-1 px-3 m-2 shadow-md`}
            >
              <p className="text-sm flex items-center">
                ...
                {/* Place Close/Open Status Display with an Open now fallback just in case no text is found in object */}
        ...
        <div className="w-[100%] h-screen flex flex-wrap justify-center items-start mx-auto pb-20 fixed bg-white top-0 right-0 box-border z-50 overflow-y-scroll">
          <div className="border-b w-full h-fit md:w-[70%] flex justify-between items-center sticky top-0 bg-white z-20">
            ...
            >
              ...
              <p>Return to Listing</p>
            </div>
            <img src={icon || "/placeholder.svg"} alt="Urban Explore" className="h-6 md:h-8 mr-3" />
            {/* --- */}
          </div>

          <div className="w-full h-full md:w-[70%] p-4 md:p-0 items-start">
            {/* Place Name */}
            <h1 className="font-bold text-lg md:text-[35px] my-2 md:my-4">{place?.name}</h1>
            {/* --- */}
            ...
            <p className="my-2 text-sm flex flex-wrap">
              ...
              {/* Place Ranking */}
              <span className="mb-1">~ {place?.ranking}</span>
              {/* --- */}

              {/* Place price level */}
              <span className="mb-1">~ {place?.price_level}</span>
              {/* --- */}
            </p>
            ...
            <div className="text-sm flex flex-wrap items-center my-2 leading-relaxed">
              ...
              {place?.phone && (
                <p className="hidden md:flex items-center mr-2">
                  ...
                  <a href={`tel:${place?.phone}`} className="underline decoration-dotted decoration-gray-400">
                    {place?.phone}
                  </a>
                  {/* --- */}
                </p>
              ...
              {place?.email && (
                <p className="hidden md:flex items-center mr-2">
                  ...
                  <a href={`mailto:${place?.email}`} className="underline decoration-dotted decoration-gray-400">
                    Email
                    <svg
            ...
            {!place?.isClose && place?.open_now_text && (
              // If Place isClose - a red button is displayed else a green button is displayed - this is possible with the difference in classes as seen below
              <button
                className={`${place.open_now_text?.includes("Closed") ? "bg-red-400" : "bg-green-400"} rounded-full py-1 px-3 my-2 mr-2`}
              >
                <p className="text-sm flex items-center">
                  <svg
                    className="h-5 w-5 mr-1"
            ...
            {/* A Mobile Only Section - Displays if either of Place Email, Phone or Website is found */}
              ...
              <div className="flex bg-white shadow-sm rounded-b p-2 md:hidden border border-t-0 mb-2">
                ...
                {place?.email && (
                  <p className="flex flex-col items-center justify-center w-full">
                    ...
                    >
                      <path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z" />
                      <path d="M18 8.118l-8 4-8-4V14a2 2 2 0 002 2h12a2 2 0 002-2V8.118z" />
                    </svg>
            ...
            <div className="md:grid grid-cols-1 md:grid-cols-3 gap-2 pb-10">
              ...
              <div className="col-span-3">
                {place?.description && (
                  <>
                    <h1 className="font-semibold text-lg md:text-2xl">About</h1>
              ...
              {(place?.awards?.length > 0 || place?.rating > 0 || place?.ranking) && (
                <div className="col-span-1 bg-white border border-gray-200 p-4 my-1 mx-0 md:my-2 shadow-sm space-y-3 md:space-y-4">
                  <h2 className="font-medium text-lg">Ratings & Recognitions</h2>
                  ...
                  {place?.ranking && <p className="my-2 text-sm">{place?.ranking}</p>}
                  {/* --- */}
                        ...
                        {place?.awards?.map((award, i) => (
                          <div key={i} className="group relative cursor-pointer">
                            <div className="flex flex-wrap items-center bg-white border border-gray-200 shadow-md rounded-md p-2 mr-2 mb-2">
                              {/* Award Image */}
                              <img src={award?.images?.small || "/placeholder.svg"} />
                              {/* --- */}
              ...
              {/* Cuisines/Special Diets - Section display only if either cuisine or diet restriction is found in place details */}
                    ...
                    <div className="text-xs space-y-2">
                      ...
                      <p>
                        ...
                        {place?.cuisine?.map((type) => (
                          <span className="bg-gray-200 rounded-full py-1 px-2 mb-2 mr-2 inline-block" key={type?.key}>
                            {type?.name}
                          </span>
                        ))}
                        {/* --- */}
              ...
              {/* Section Displays only if either of Place Email, Phone or Website is found */}
              {(place?.phone || place?.email || place?.website) && (
                <div className="col-span-1 bg-white border border-gray-200 p-4 my-1 mx-0 md:my-2 shadow-sm space-y-3 md:space-y-4">
