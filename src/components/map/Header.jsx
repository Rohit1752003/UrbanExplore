...
const Header = ({ setCoordinates }) => {
  ...
  // Sets new AutoComplete state value after loading search result value
  const onLoad = (autoC) => setAutoComplete(autoC)

  // Function that receives and sets new Coordinates for new search location
  const onPlaceChanged = () => {
    // getting 'longitude' and 'latitude' autoComplete search value
    const lat = autoComplete.getPlace().geometry.location.lat()
    const lng = autoComplete.getPlace().geometry.location.lng()

    // Set new coordinates received from autoComplete
  ...
  return (
    <div className="flex items-center w-full p-2 absolute top-0 z-10">
      <div className="flex bg-white justify-between items-center w-full p-3 md:p-4 rounded-sm shadow-md">
        ...
        {!search && <img src={logo || "/placeholder.svg"} alt="Urban Explore" className="h-6 sm:h-7 md:h-8" />}
        {/* --- */}
          ...
          <Autocomplete onLoad={onLoad} onPlaceChanged={onPlaceChanged}>
            <div className="group relative">
              ...
              <input
                ...
                type="text"
                placeholder="Search urban destinations..."
              />
            </div>
          </Autocomplete>
          {/* --- */}
